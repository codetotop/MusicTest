...
import com.gem.mpi.screen.adapter.OnItemClickListener;
...
public class BestSurveyFragment extends ViewFragment<BestSurveyContract.Presenter> implements BestSurveyContract.View {
  ...
  @Override
  public void updateLocationView(LocationModel locationModel) {
    if (isAdded()) {
      mLocationTv.setText(getString(R.string.home_label_location, locationModel.getName()));
      updateDateView(locationModel);
    }
  }
  ...
  private void updateDateView() {
    LocationDTO locationDTO = PrefWrapper.getLocationSetting(getContext());
    updateDateView(LocationMapper.transform(locationDTO));
  }

  private void updateDateView(LocationModel locationModel) {
    SimpleDateFormat df = new SimpleDateFormat("MMM dd", Locale.US);
    LocationModel.ShiftModel shift = CommonUtils.getShift(locationModel);
